...
public class Environment {
    ...
    // Verifies the state of Game Over
    ...
    public void spawnEnemy() {
        if (agents.size() < maxEnemies) {
            boolean validPosition = false;
            int enemyPosX = 0, enemyPosY = 0;

            // Tries to find a valid position to spawn
            while (!validPosition) {
    ...
    // Update the enemies on screen
    public void updateEnemies() {
        long currentTime = System.currentTimeMillis();

        if (currentTime - lastSpawnTime >= spawnInterval) {
            spawnEnemy();
            lastSpawnTime = currentTime; // Update time from last spawn
        }
    }
